...
const BACKEND_PATH = resolve(__dirname, 'ictrl_be');
const STATIC_FILES_PATH = resolve(__dirname, 'ictrl_be/client');
...
const main = async () => {
  const mainPort = DEBUG_PORT || await getFreePort();
  const localAuthKey = randomUUID();
  const appURL = `http${DEBUG_PORT
      ? ''
      : 's'}://127.0.0.1:${mainPort}/dashboard`;
...
// the backend process handle
  ...
  if (IS_MAC) {
    if (!DEBUG_PORT) {
      ictrl_be = spawn('./ictrl_be', [mainPort, localAuthKey], {
        cwd: BACKEND_PATH, shell: true, // FIXME: needs to be specified since Electron 15.
        // reasons unknown
      });
    }
  ...
  const interceptFilePaths = () => {
    ...
    protocol.interceptFileProtocol('file', (request, callback) => {
      ...
      if (request.method !== 'GET' || fileName.includes('ictrl_be') ||
        ...
        callback(resolve(STATIC_FILES_PATH, fileName));
      }
    });
  };
  ...
  const createDashboardWindow = () => {
    ...
    if (backendReady) {
      ...
      const tempURL = `file://${resolve(STATIC_FILES_PATH, 'index.html')}`;
